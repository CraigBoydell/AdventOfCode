...
$ProcessedIDScans = ($IDScans -split "`n`n") | select-object -property @{N='ScannedID';E={$_ -split "`n" -join ' '}} | select-object -Property *,@{N='AvailableAttributes';E={$_.ScannedID -split ' ' | foreach-object { ($_ -split ':')[0]  }}} | Select-Object -Property *,@{N='AttributeCountMinusCID';E={@($_.AvailableAttributes | where-object {$_ -ne 'cid'}).count }}
$ValidPassports = $ProcessedIDScans | where-object {$_.AttributeCountMinusCID -eq 7}
$ValidPassports = $ValidPassports | Select-Object -Property *,@{N='JSON';E={'{"' + $_.ScannedID.replace(' ','","').replace(':','":"') + '"}'}}
$ValidPassports = $ValidPassports | select-object -property *,@{N='PatternTest';E={$Example = $_.JSON | convertfrom-JSON; ($Example | get-member | where-object {$_.MemberType -eq 'NoteProperty'}).Name | foreach { $Example.$_ -match $patterns.$_  }}}
$NumberOfValidPassports = ($ValidPassports | where-object {$_.PatternTest -notcontains $false}).count
