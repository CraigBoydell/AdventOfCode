...
'@
$Test = ($String -split "`n") | Select-Object -Property @{N='MatchCount';E={invoke-command -ScriptBlock $([scriptblock]::Create($($_.split(' ')[0]).replace('-','..')) )}}, @{N='Pattern';E={$($_.split(' ')[1]).replace(':','')}},@{N='Password';E={$_.split(' ')[2]}} | Select-Object -Property *,@{N='Grouped';E={$_.Password.ToCharArray() | group-Object}}
$PasswordCheckResult = $Test | Select-Object -Property *,@{N='PatternInGroup';E={if ($_.Pattern -in $_.Grouped.Name) {$Pattern = $_.Pattern; $_.Grouped | where-object {$_.Name -eq $Pattern}} }} | select-object -Property *,@{N='ValidPassword';E={$_.PatternInGroup.count -in $_.MatchCount }}
$PasswordCheckResult | Group-Object -Property ValidPassword