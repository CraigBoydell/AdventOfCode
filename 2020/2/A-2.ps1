...
'@
$Test = ($String -split "`n") | Select-Object -Property @{N='MatchCount';E={invoke-command -ScriptBlock $([scriptblock]::Create($($_.split(' ')[0]).replace('-',',')) )}}, @{N='Pattern';E={$($_.split(' ')[1]).replace(':','')}},@{N='Password';E={$_.split(' ')[2]}} | Select-Object -Property *,@{N='Grouped';E={$_.password[$($_.MatchCount | foreach-object {$_ - 1}) ]   }}
$PasswordCheckResult = $Test | select-object -property *,@{N='ValidPassword';E={$ToTest = $_; $($_.Grouped | where-object {$_ -eq $ToTest.Pattern}).count -eq 1} }
