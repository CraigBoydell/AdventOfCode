...
$CurrentPath = '/'; $CurrentFolder = ''; $FullFS = for ($y=0;$y -lt $ConsoleCommands.count; $y++) {
 ...
 $AllFolders | Select-Object -Property *,@{N='TotalSize';E={ $Test = $_.ParentFolderPath; ($FullFS |where-object {$_.FullName.StartsWith($Test)} | Measure-Object -Property Size -Sum).sum }} | where-object {$_.TotalSize -le 100000} | Measure-Object -Property TotalSize -sum