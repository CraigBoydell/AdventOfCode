...
$WinningBoards = New-Object -TypeName "System.Collections.ArrayList"

foreach ($num in $seq) {
    for ($b=0;$b -lt $ModBoards.count;$b++) {
      if ($b -notin $WinningBoards.BoardNumber) {
        ...
        if ((Test-BoardCols -Board $ModBoards[$b]) -eq $True -or (Test-BoardRows -Board $ModBoards[$b]) -eq $True ) {
            ...
            $WinningBoards.Add($GameResult) | out-null
            #break
        }
      }
    }
    ...
    #>
    if ($WinningBoards.count -eq $ModBoards.count){
      Break
    }
}

$WinningBoards[-1]
[int]$WinningBoards[-1].LastNumber * [int]$WinningBoards[-1].UnmarkedSum