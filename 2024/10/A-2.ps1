...
(($StartPoints | foreach { $StartPoint = $_; (@('U','D','L','R') | foreach { get-NextStep -CurrentStepX $StartPoint.x -CurrentStepY $StartPoint.y -Direction $_} | where {$_.Height -eq '9'}).count}) | measure-object -sum).sum
