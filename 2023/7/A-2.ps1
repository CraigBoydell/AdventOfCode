...
Function Test-Hand {
    ...
    $TestHand = $Hand.hand.ToCharArray() | Group-Object | Group-Object -Property Count
...
Function Test-Hand {
    ...
    if ($Hand.hand -like '*J*') {
        $Grouped = $Hand.Hand.tochararray() | Group-Object
        if (($Grouped | where {$_.Name -ne 'J'} | where {$_.Count -gt 1}).count -gt 0) {
            $BestCard = ($Grouped | where {$_.Name -ne 'J'} | Sort-Object -Property count -Descending)[0].name
        } else {
            $BestCard = ($Grouped | where {$_.Name -ne 'J'} | select-object -Property *,@{N='CardRank';E={$Global:HTCardRanks[[string]$_.Name]}} | sort-object -Property CardRank -Descending | Select-Object -first 1).name
        }
        if ( @($Grouped | where {$_.Name -eq 'J' -and $_.Count -eq 5}).count -eq 1 ){} else { $Hand.Hand = $Hand.Hand.Replace('J',$BestCard)}
    }
...
$Global:HTCardRanks = @{}
$CardRanks | foreach {$Global:HTCardRanks[$_.Card] = $_.Rank}
...
$Hands = $Hands | Select-Object -Property *,@{N='Type';E={Test-Hand -Hand $_}}
...
$global:t = 0; $Ranked = $GroupedHands | foreach {  
    ...
    do {          
        ...
        for ($i=0; $i -lt $GroupToProcess.count-1; $i++) {
          ...
          $TestResult = $ComparedHands.hand.tochararray()[$c,($c+5)] |Select-Object -Property @{N='Card';E={$_}},@{N='Value';E={$Global:HTCardRanks[[string]$_]}},@{N='Index';E={$global:index;$Global:index++}}
