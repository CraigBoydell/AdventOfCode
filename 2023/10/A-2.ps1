...
do {
    ...
    #$PathTracker.Add(@($Global:Sx,$Global:sy)) | Out-Null
#    $Checker | FT
...
$PathTracker.Add(@($Global:Sx,$Global:sy,$step)) | Out-Null
#'Ex: ' + $ExcludeStep
#'Next: ' + $Step
...
$PathTracker | foreach {$Grid[$_[0]][$_[1]] = $(if ($_[2] -in @('D','U') ) {'|'} else {'-'} ) }

$AllTiles = 0

for ($y=0;$y -lt $Grid.count; $y++) {
    #$SplitLine = (($Grid | foreach {$_ -join ''})[$y]).split('P')
    for ($t =0; $t -lt $Grid[$y].count; $t++) {
      if ($Grid[$y][$t] -notin @('-','|')) {
        $CountN = $($Grid[$y][0..$($t-1)] | where {$_ -eq '|'}).count
        if ($CountN%2 -eq 1) {$AllTiles++}
      }
    }
}
