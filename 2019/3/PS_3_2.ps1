...
$Line1 | ForEach-Object {
  $i++
  write-progress -activity $('Processing Line 1') -percentComplete $([math]::Floor(($i / $Line1.count) * 100))
  $Step = $([int]$_.Substring(1,$_.Length-1))
  #$Step
  switch (($_).Substring(0,1))
  {
    R {1..$Step | ForEach-Object { $n++; $Global:x++; $Global:Canvas."$Global:x,$Global:y" = $('a'+$n)  }}
    L {1..$Step | ForEach-Object { $n++; $Global:x--; $Global:Canvas."$Global:x,$Global:y" = $('a'+$n)  }}
    U {1..$Step | ForEach-Object { $n++; $Global:y++; $Global:canvas."$Global:x,$Global:y" = $('a'+$n)  }}
    D {1..$Step | ForEach-Object { $n++; $Global:y--; $Global:canvas."$Global:x,$Global:y" = $('a'+$n)  }}
  }
}
...
$Line2 | ForEach-Object {
  $i++
  write-progress -activity $('Processing Line 2') -percentComplete $([math]::Floor(($i / $Line2.count) * 100))
  $Step = $([int]$_.Substring(1,$_.Length-1))
  switch (($_).Substring(0,1))
  {
    R {1..$Step | ForEach-Object { $n++; $global:x++; if (($Global:Canvas).ContainsKey("$global:x,$global:y")) { if ($Global:canvas."$global:x,$global:y" -like 'a*') {$Global:canvas."$global:x,$global:y" = $($Global:canvas."$global:x,$global:y" + 'b' + $n); $Global:Intersects."$global:x,$global:y" = $Global:canvas."$global:x,$global:y"} } else {$Global:canvas."$global:x,$global:y" = $('b' + $n)} }}
    L {1..$Step | ForEach-Object { $n++; $global:x--; if (($Global:Canvas).ContainsKey("$global:x,$global:y")) { if ($Global:canvas."$global:x,$global:y" -like 'a*') {$Global:canvas."$global:x,$global:y" = $($Global:canvas."$global:x,$global:y" + 'b' + $n); $Global:Intersects."$global:x,$global:y" = $Global:canvas."$global:x,$global:y"} } else {$Global:canvas."$global:x,$global:y" = $('b' + $n)} }}
    U {1..$Step | ForEach-Object { $n++; $global:y++; if (($Global:Canvas).ContainsKey("$global:x,$global:y")) { if ($Global:canvas."$global:x,$global:y" -like 'a*') {$Global:canvas."$global:x,$global:y" = $($Global:canvas."$global:x,$global:y" + 'b' + $n); $Global:Intersects."$global:x,$global:y" = $Global:canvas."$global:x,$global:y"} } else {$Global:canvas."$global:x,$global:y" = $('b' + $n)} }}
    D {1..$Step | ForEach-Object { $n++; $global:y--; if (($Global:Canvas).ContainsKey("$global:x,$global:y")) { if ($Global:canvas."$global:x,$global:y" -like 'a*') {$Global:canvas."$global:x,$global:y" = $($Global:canvas."$global:x,$global:y" + 'b' + $n); $Global:Intersects."$global:x,$global:y" = $Global:canvas."$global:x,$global:y"} } else {$Global:canvas."$global:x,$global:y" = $('b' + $n)} }}
  }
}
